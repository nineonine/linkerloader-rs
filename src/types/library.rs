...
use crate::types::object::{MAGIC_NUMBER, parse_object_file, ObjectIn};
...
impl StaticLib {
    ...
    fn parse_file_lib(path: &str) -> Result<Self, LibError> {
        ...
        let (num_of_mods, lib_dir_offset) =
            match hdr.as_slice() {
        ...
        for i in 0..num_of_mods {
            let mod_entry: Vec<&str> = file_lines[lib_dir_offset+i-1].split(' ').map(|s| s.trim()).collect();

...
impl ObjectOut {
    ...
    pub fn ppr(&self) -> String {
        ...
        s.push_str(MAGIC_NUMBER);s.push_str("-OUT\n");
        ...
        for segment_name in segment_order.iter() {
            if let Some(seg) = self.segments.get(segment_name) {
                segs.push(format!("{} {:X} {:X}", segment_name, seg.segment_start, seg.segment_len));
                if let Some(segment_data) = self.object_data.get(segment_name) {
                code_and_data.push(format!("  Obj code/data len: {:X} {}", segment_data.len(), segment_name));
                }
            }
        }
